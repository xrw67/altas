...
struct MockConnection : public Conn {
  std::vector<cppboot::bus::MsgPtr> output_msgs;  // 发送出去的msg
...
TEST(BusClient, should_call_and_reply) {
  ...
  cppboot::bus::BusClient client("name1", mock_conn);
...
TEST(BusClient, should_reply_when_call_by_other_client) {
  ...
  cppboot::bus::BusClient client1("name1", conn1);
  cppboot::bus::BusClient client2("name1", conn2);

...
const size_t npos = cppboot::string_view::npos;
...
TEST(StringViewTest, Ctor) {
...

  {
    ...
    cppboot::string_view s20(hello);
    EXPECT_TRUE(s20.data() == hello);
    EXPECT_EQ(5, s20.length());

    // const char* with length.
    cppboot::string_view s21(hello, 4);
    EXPECT_TRUE(s21.data() == hello);
    EXPECT_EQ(4, s21.length());

    // Not recommended, but valid C++
    cppboot::string_view s22(hello, 6);
...

  {
    ...
    cppboot::string_view s1(str1);
    s1.remove_prefix(strlen("BUNGIE-JUM"));

    cppboot::string_view s2(str2);
...
TEST(StringViewTest, Swap) {
  cppboot::string_view a("a");
  cppboot::string_view b("bbb");
...
TEST(StringViewTest, STLComparator) {
  ...
  cppboot::string_view p1(s1);
  cppboot::string_view p2(s2);
  cppboot::string_view p3(s3);

  typedef std::map<cppboot::string_view, int> TestMap;
  TestMap map;
...
TEST(StringViewTest, ComparisonOperators) {
  COMPARE(true, ==, "", "");
  COMPARE(true, ==, "", cppboot::string_view());
  COMPARE(true, ==, cppboot::string_view(), "");
  COMPARE(true, ==, "a", "a");
  COMPARE(true, ==, "aa", "aa");
  COMPARE(false, ==, "a", "");
...
TEST(StringViewTest, STL1) {
  const cppboot::string_view a("abcdefghijklmnopqrstuvwxyz");
  const cppboot::string_view b("abc");
  const cppboot::string_view c("xyz");
  const cppboot::string_view d("foobar");
  const cppboot::string_view e;
  std::string temp("123");
  temp += '\0';
  temp += "456";
  const cppboot::string_view f(temp);
...
TEST(StringViewTest, STL2) {
  ...
  const cppboot::string_view b("abc");
  const cppboot::string_view c("xyz");
  cppboot::string_view d("foobar");
  const cppboot::string_view e;
  const cppboot::string_view f(
      "123"
      "\0"
      "456",
      7);

  d = cppboot::string_view();
  EXPECT_EQ(d.size(), 0);
  EXPECT_TRUE(d.empty());
  ...
  cppboot::string_view g("xx not found bb");
...
TEST(StringViewTest, TruncSubstr) {
  const cppboot::string_view hi("hi");
...
TEST(StringViewTest, UTF8) {
  ...
  EXPECT_EQ(utf8_len, cppboot::string_view(utf8_twice).find_first_of(" "));
  EXPECT_EQ(utf8_len, cppboot::string_view(utf8_twice).find_first_of(" \t"));
}
...
TEST(StringViewTest, FindConformance) {
  ...
  for (const auto& s : specs) {
    ...
    cppboot::string_view sp = s.haystack;
...
TEST(StringViewTest, Remove) {
  cppboot::string_view a("foobar");
  std::string s1("123");
  s1 += '\0';
  s1 += "456";
  cppboot::string_view e;
  std::string s2;

  // remove_prefix
  cppboot::string_view c(a);
  c.remove_prefix(3);
  EXPECT_EQ(c, "bar");
  c = a;
  c.remove_prefix(0);
...
TEST(StringViewTest, Set) {
  cppboot::string_view a("foobar");
  cppboot::string_view empty;
  cppboot::string_view b;

  // set
  b = cppboot::string_view("foobar", 6);
  EXPECT_EQ(b, a);
  b = cppboot::string_view("foobar", 0);
  EXPECT_EQ(b, empty);
  b = cppboot::string_view("foobar", 7);
  EXPECT_NE(b, a);

  b = cppboot::string_view("foobar");
  EXPECT_EQ(b, a);
}

TEST(StringViewTest, FrontBack) {
  static const char arr[] = "abcd";
  const cppboot::string_view csp(arr, 4);
...
TEST(StringViewTest, FrontBackSingleChar) {
  ...
  const cppboot::string_view csp(&c, 1);
...
// `std::cppboot::string_view::cppboot::string_view(const char*)` calls
...
#define CPPBOOT_HAVE_STRING_VIEW_FROM_NULLPTR 1
#endif

TEST(StringViewTest, NULLInput) {
  cppboot::string_view s;
  EXPECT_EQ(s.data(), nullptr);
  EXPECT_EQ(s.size(), 0);

#ifdef CPPBOOT_HAVE_STRING_VIEW_FROM_NULLPTR
  s = cppboot::string_view(nullptr);
  EXPECT_EQ(s.data(), nullptr);
  EXPECT_EQ(s.size(), 0);

  // .ToString() on a cppboot::string_view with nullptr should produce the empty
  // string.
  EXPECT_EQ("", std::string(s));
#endif  // CPPBOOT_HAVE_STRING_VIEW_FROM_NULLPTR
}
...
TEST(StringViewTest, Comparisons2) {
  ...
  cppboot::string_view abc("abcdefghijklmnopqrstuvwxyz");

  // check comparison operations on strings longer than 4 bytes.
  EXPECT_EQ(abc, cppboot::string_view("abcdefghijklmnopqrstuvwxyz"));
  EXPECT_EQ(abc.compare(cppboot::string_view("abcdefghijklmnopqrstuvwxyz")), 0);

  EXPECT_LT(abc, cppboot::string_view("abcdefghijklmnopqrstuvwxzz"));
  EXPECT_LT(abc.compare(cppboot::string_view("abcdefghijklmnopqrstuvwxzz")), 0);

  EXPECT_GT(abc, cppboot::string_view("abcdefghijklmnopqrstuvwxyy"));
  EXPECT_GT(abc.compare(cppboot::string_view("abcdefghijklmnopqrstuvwxyy")), 0);

  // The "substr" variants of `compare`.
  cppboot::string_view digits("0123456789");

  // Taking cppboot::string_view
  EXPECT_EQ(digits.compare(3, npos, cppboot::string_view("3456789")), 0);  // 2
  EXPECT_EQ(digits.compare(3, 4, cppboot::string_view("3456")), 0);        // 2
  EXPECT_EQ(digits.compare(10, 0, cppboot::string_view()), 0);             // 2
  EXPECT_EQ(digits.compare(3, 4, cppboot::string_view("0123456789"), 3, 4),
            0);  // 3
  EXPECT_LT(digits.compare(3, 4, cppboot::string_view("0123456789"), 3, 5),
            0);  // 3
  EXPECT_LT(digits.compare(0, npos, cppboot::string_view("0123456789"), 3, 5),
            0);  // 3
  // Taking const char*
...
TEST(StringViewTest, ExplicitConversionOperator) {
  cppboot::string_view sp = "hi";
  EXPECT_EQ(sp, std::string(sp));
}

TEST(StringViewTest, NullSafeStringView) {
  {
    cppboot::string_view s = cppboot::NullSafeStringView(nullptr);
    EXPECT_EQ(nullptr, s.data());
    EXPECT_EQ(0, s.size());
    EXPECT_EQ(cppboot::string_view(), s);
  }
  {
    static const char kHi[] = "hi";
    cppboot::string_view s = cppboot::NullSafeStringView(kHi);
    EXPECT_EQ(kHi, s.data());
    EXPECT_EQ(strlen(kHi), s.size());
    EXPECT_EQ(cppboot::string_view("hi"), s);
  }
}

TEST(StringViewTest, ConstexprNullSafeStringView) {
  {
    constexpr cppboot::string_view s = cppboot::NullSafeStringView(nullptr);
    EXPECT_EQ(nullptr, s.data());
    EXPECT_EQ(0, s.size());
    EXPECT_EQ(cppboot::string_view(), s);
  }
#if !defined(_MSC_VER) || _MSC_VER >= 1910
  // MSVC 2017+ is required for good constexpr cppboot::string_view support.
  // See the implementation of `cppboot::string_view::StrlenInternal()`.
  {
  ...
  {
    constexpr cppboot::string_view s = cppboot::NullSafeStringView("hello");
    EXPECT_EQ(s.size(), 5);
    EXPECT_EQ("hello", s);
  }
#endif
...
TEST(StringViewTest, ConstexprCompiles) {
  constexpr cppboot::string_view sp;
#ifdef CPPBOOT_HAVE_STRING_VIEW_FROM_NULLPTR
  constexpr cppboot::string_view cstr(nullptr);
#endif
  constexpr cppboot::string_view cstr_len("cstr", 4);

// This duplicates the check for __builtin_strlen in the header.
#if CPPBOOT_HAVE_BUILTIN(__builtin_strlen) || \
...
#ifdef CPPBOOT_HAVE_CONSTEXPR_STRING_VIEW_FROM_CSTR
  constexpr cppboot::string_view cstr_strlen("foo");
  EXPECT_EQ(cstr_strlen.length(), 3);
  constexpr cppboot::string_view cstr_strlen2 = "bar";
  EXPECT_EQ(cstr_strlen2, "bar");

#if CPPBOOT_HAVE_BUILTIN(__builtin_memcmp) || \
    (defined(__GNUC__) && !defined(__clang__))
#define CPPBOOT_HAVE_CONSTEXPR_STRING_VIEW_COMPARISON 1
#endif
#ifdef CPPBOOT_HAVE_CONSTEXPR_STRING_VIEW_COMPARISON
  constexpr cppboot::string_view foo = "foo";
  constexpr cppboot::string_view bar = "bar";
  ...
  constexpr cppboot::string_view::iterator const_begin_empty = sp.begin();
  constexpr cppboot::string_view::iterator const_end_empty = sp.end();
  EXPECT_EQ(const_begin_empty, const_end_empty);

#ifdef CPPBOOT_HAVE_STRING_VIEW_FROM_NULLPTR
  constexpr cppboot::string_view::iterator const_begin_nullptr = cstr.begin();
  constexpr cppboot::string_view::iterator const_end_nullptr = cstr.end();
  EXPECT_EQ(const_begin_nullptr, const_end_nullptr);
#endif  // CPPBOOT_HAVE_STRING_VIEW_FROM_NULLPTR
#endif  // !defined(__clang__) || ...

  constexpr cppboot::string_view::iterator const_begin = cstr_len.begin();
  constexpr cppboot::string_view::iterator const_end = cstr_len.end();
  constexpr cppboot::string_view::size_type const_size = cstr_len.size();
  constexpr cppboot::string_view::size_type const_length = cstr_len.length();
...
constexpr char ConstexprMethodsHelper() {
  ...
  cppboot::string_view str("123", 3);
  str.remove_prefix(1);
  str.remove_suffix(1);
  cppboot::string_view bar;
  str.swap(bar);
  return bar.front();
#else
  return '2';
...
TEST(StringViewTest, ConstexprMethods) {
  ...
  // substr
  constexpr cppboot::string_view foobar("foobar", 6);
  constexpr cppboot::string_view foo = foobar.substr(0, 3);
  constexpr cppboot::string_view bar = foobar.substr(3);
  EXPECT_EQ(foo, "foo");
  EXPECT_EQ(bar, "bar");
}

TEST(StringViewTest, Noexcept) {
  EXPECT_TRUE((std::is_nothrow_constructible<cppboot::string_view,
                                             const std::string&>::value));
  EXPECT_TRUE((std::is_nothrow_constructible<cppboot::string_view,
                                             const std::string&>::value));
  EXPECT_TRUE(std::is_nothrow_constructible<cppboot::string_view>::value);
  constexpr cppboot::string_view sp;
...
TEST(ComparisonOpsTest, HeterogeneousStringViewEquals) {
  EXPECT_EQ(cppboot::string_view("hello"), std::string("hello"));
  EXPECT_EQ("hello", cppboot::string_view("hello"));
}

TEST(FindOneCharTest, EdgeCases) {
  cppboot::string_view a("xxyyyxx");
...
TEST(NonNegativeLenTest, NonNegativeLen) {
  EXPECT_DEATH_IF_SUPPORTED(cppboot::string_view("xyz", -1), "len <= kMaxSize");
}

TEST(LenExceedsMaxSizeTest, LenExceedsMaxSize) {
  auto max_size = cppboot::string_view().max_size();

  // This should construct ok (although the view itself is obviously invalid).
  cppboot::string_view ok_view("", max_size);

  // Adding one to the max should trigger an assertion.
  EXPECT_DEATH_IF_SUPPORTED(cppboot::string_view("", max_size + 1),
                            "len <= kMaxSize");
}
...
TEST_F(StringViewStreamTest, ResetsWidth) {
  ...
  // If we weren't resetting width after formatting the cppboot::string_view,

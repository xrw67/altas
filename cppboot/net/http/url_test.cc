...
TEST(Url, Default) {
  cppboot::http::Url url("");
  ASSERT_FALSE(url.IsValid());
}

TEST(Url, Copy) {
  cppboot::http::Url url1("http://bing.com/search?q=dotnet");
  cppboot::http::Url url2 = url1;
...
TEST(Url, ParseWithoutQuery) {
  cppboot::http::Url url("http://127.0.0.1:59999/name");
...
TEST(Url, ParseHttp) {
  cppboot::http::Url url("http://bing.com/search?q=dotnet");
  ASSERT_TRUE(url.IsValid());
...
TEST(Url, ParseHttps) {
  cppboot::http::Url url("https://bing.com:9000/search?q=dotnet");
  ASSERT_TRUE(url.IsValid());
